...
# Hardcoded: abilita sempre Cloudflare quick tunnel + auto install cloudflared
os.environ["COMFYUI_ENABLE_CLOUDFLARE"] = "1"
os.environ["COMFYUI_AUTO_INSTALL_CLOUDFLARED"] = "1"
extra_packages = [
     "requests",
...
# Hardcoded: Nexus auto-install / auto-update
NEXUS_AUTO_INSTALL = True
NEXUS_AUTO_UPDATE = False   # metti True se vuoi fare git pull a ogni avvio
NEXUS_FORCE_CORS = True     # Nexus remoto/multiutente: consigliato
NEXUS_REPO_URL = "https://github.com/daxcay/ComfyUI-Nexus.git"
NEXUS_DIRNAME = "ComfyUI-Nexus"

# Hardcoded Nexus admins (sovrascrive admins.json a ogni avvio)
NEXUS_FORCE_ADMINS_JSON = True
NEXUS_DEFAULT_ADMINS = {
    "admin1": "pass1",
    "admin2": "pass2"
}


def _run_cmd_quiet(cmd, cwd=None):
    try:
        subprocess.check_call(cmd, cwd=cwd)
        return True
    except Exception as e:
        print(f"[NEXUS] Command failed: {cmd} -> {e}")
        return False

import json

def ensure_nexus_admins_json():
    """
    Scrive (o sovrascrive) il file admins.json di Nexus con un JSON hardcoded.
    """
    if not NEXUS_FORCE_ADMINS_JSON:
        return

    base_dir = os.path.dirname(os.path.realpath(__file__))

    # Nexus README cita ComfyUI/nexus/admins.json
    # Alcune installazioni possono avere la cartella nel custom node
    candidate_paths = [
        os.path.join(base_dir, "nexus", "admins.json"),
        os.path.join(base_dir, "custom_nodes", "ComfyUI-Nexus", "nexus", "admins.json"),
    ]

    # Scegli il primo path esistente; se nessuno esiste, crea quello "root" (README-style)
    target_path = None
    ...
    try:
        with open(tmp_path, "w", encoding="utf-8") as f:
            json.dump(NEXUS_DEFAULT_ADMINS, f, ensure_ascii=False, indent=2)
            f.write("\n")
        os.replace(tmp_path, target_path)
        print(f"[NEXUS] admins.json forzato in: {target_path}")
    except Exception as e:
        try:
            ...
            pass
        print(f"[NEXUS] Errore scrittura admins.json: {e}")


def ensure_comfyui_nexus_installed():
    """
    Clona ComfyUI-Nexus dentro custom_nodes prima del bootstrap requirements.
    In questo modo il tuo auto_install_requirements() installerà anche eventuali dipendenze del nodo.
    """
    if not NEXUS_AUTO_INSTALL:
        return
    ...
    nexus_dir = os.path.join(custom_nodes_dir, NEXUS_DIRNAME)
    nexus_disabled_dir = nexus_dir + ".disabled"

    # Se l'utente l'ha disabilitato esplicitamente, rispetta la scelta
    if os.path.isdir(nexus_disabled_dir):
        print(f"[NEXUS] Trovato {nexus_disabled_dir} (disabled), skip install.")
        return

    # Se già presente
    if os.path.isdir(nexus_dir):
        print(f"[NEXUS] Già presente: {nexus_dir}")
        if NEXUS_AUTO_UPDATE and shutil.which("git"):
            print("[NEXUS] Auto-update attivo, eseguo git pull...")
            _run_cmd_quiet(["git", "-C", nexus_dir, "pull", "--ff-only"])
        return

    # Prova con git clone
    if shutil.which("git"):
        print(f"[NEXUS] Clono repository in {nexus_dir}")
        ok = _run_cmd_quiet(["git", "clone", NEXUS_REPO_URL, nexus_dir])
        if ok:
            return
        print("[NEXUS] git clone fallito, provo fallback ZIP...")

    # Fallback ZIP (senza git)
    try:
        import urllib.request
        import zipfile
        import io

        zip_url = "https://github.com/daxcay/ComfyUI-Nexus/archive/refs/heads/master.zip"
        print(f"[NEXUS] Download ZIP fallback: {zip_url}")
        data = urllib.request.urlopen(zip_url, timeout=60).read()

        with zipfile.ZipFile(io.BytesIO(data)) as zf:
            extract_tmp = os.path.join(custom_nodes_dir, "_nexus_extract_tmp")
            ...
            # di solito crea ComfyUI-Nexus-master
            extracted_root = None
            ...
            if not extracted_root:
                raise RuntimeError("Cartella estratta non trovata")

            if os.path.isdir(nexus_dir):
                shutil.rmtree(nexus_dir, ignore_errors=True)
            shutil.move(extracted_root, nexus_dir)
            shutil.rmtree(extract_tmp, ignore_errors=True)

        print(f"[NEXUS] Installazione ZIP completata: {nexus_dir}")

    except Exception as e:
        print(f"[NEXUS] Installazione fallback ZIP fallita: {e}")


def force_nexus_runtime_flags():
    """
    Forza flag runtime utili per Nexus prima del parsing args di ComfyUI.
    """
    if not NEXUS_FORCE_CORS:
        return

    # Evita duplicati
    if "--enable-cors-header" not in sys.argv:
        sys.argv.extend(["--enable-cors-header", "*"])
        print('[NEXUS] Forzato flag: --enable-cors-header "*"')

def _maybe_install_cloudflared():
    """
    Restituisce il path del binario cloudflared.
    Se non esiste, lo scarica in una cartella locale (senza sudo/dpkg).
    """
    # Se già nel PATH, usa quello
    system_bin = shutil.which("cloudflared")
    if system_bin:
        return system_bin

    # Cartella locale accanto a main.py
    base_dir = os.path.dirname(os.path.realpath(__file__))
    local_bin_dir = os.path.join(base_dir, ".local_bin")
    local_bin = os.path.join(local_bin_dir, "cloudflared")

    # Se già scaricato localmente, usa quello
    ...
    # (Hai già hardcoded l'env, quindi questo check può restare o essere rimosso)
    if os.environ.get("COMFYUI_AUTO_INSTALL_CLOUDFLARED", "1") != "1":
        logging.info("cloudflared non trovato e auto-install disabilitato.")
        return None

    if not (sys.platform.startswith("linux") and platform.machine().lower() in ("x86_64", "amd64")):
        logging.warning("Auto-install cloudflared locale supportato qui solo su Linux amd64/x86_64.")
        return None

    # Binario standalone (NON .deb)
    bin_url = "https://github.com/cloudflare/cloudflared/releases/latest/download/cloudflared-linux-amd64"
    tmp_path = local_bin + ".part"

    try:
        os.makedirs(local_bin_dir, exist_ok=True)

        logging.info(f"Scarico cloudflared (binario locale) da: {bin_url}")
        urllib.request.urlretrieve(bin_url, tmp_path)

        os.chmod(tmp_path, 0o755)
        os.replace(tmp_path, local_bin)

        # Test rapido
        try:
            out = subprocess.check_output([local_bin, "--version"], text=True, stderr=subprocess.STDOUT)
            logging.info(f"cloudflared locale pronto: {out.strip()}")
        except Exception as e:
            logging.warning(f"cloudflared scaricato ma test --version fallito: {e}")
        ...
        try:
            ...
            pass
        logging.error(f"Installazione locale cloudflared fallita: {e}")
        return None

def _wait_for_local_port(host: str, port: int, timeout_s: float = 300.0, poll_s: float = 0.5) -> bool:
    """
    Attende finché la porta locale non è raggiungibile.
    """
    start = time.time()
...
def _cloudflared_tunnel_worker(host: str, port: int):
    """
    Aspetta che ComfyUI sia pronto e poi avvia cloudflared quick tunnel.
    """
    global _CLOUDFLARED_PROCESS

    if not _wait_for_local_port(host, port):
        logging.error(f"Cloudflared: timeout in attesa di ComfyUI su {host}:{port}")
        return

    logging.info("ComfyUI pronto, avvio cloudflared quick tunnel...")

    cloudflared_bin = _maybe_install_cloudflared()
    if not cloudflared_bin:
        logging.warning("Cloudflared non disponibile: tunnel non avviato.")
        return
    ...
    try:
        # cloudflared spesso scrive su stderr; unisco tutto su stdout per semplificare il parsing
        ...
        for line in _CLOUDFLARED_PROCESS.stdout:
            ...
            if match and not url_printed:
                ...
                print(f"\n🌍 URL pubblico ComfyUI (Cloudflare): {public_url}\n")
                logging.info(f"Cloudflare tunnel URL: {public_url}")
                url_printed = True

            # opzionale: logga output cloudflared (può essere verboso)
            logging.info(f"[cloudflared] {line}")

    except FileNotFoundError:
        logging.error("cloudflared non trovato nel PATH.")
    except Exception as e:
        logging.error(f"Errore durante l'avvio di cloudflared: {e}")


def start_cloudflared_tunnel(port: int, host: str = "127.0.0.1"):
    """
    Avvia un thread daemon che aspetta ComfyUI e apre un quick tunnel Cloudflare.
    """
    t = threading.Thread(
        target=_cloudflared_tunnel_worker,
        daemon=True,
        args=(host, int(port)),
    )
    t.start()
...
ensure_comfyui_nexus_installed()
ensure_nexus_admins_json()
auto_install_requirements()


force_nexus_runtime_flags()
...
def apply_shared_model_paths():
    ...
    model_roots = [
        ...
        r"/vscode/workspace/ComfyUI/models",     # seconda cartella modelli (solo lettura/logica di scansione)
    ]
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Cloudflare quick tunnel (opzionale)
        if os.environ.get("COMFYUI_ENABLE_CLOUDFLARE", "0") == "1":
            start_cloudflared_tunnel(port=args.port, host="127.0.0.1")
